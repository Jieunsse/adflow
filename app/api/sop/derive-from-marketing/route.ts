...
export async function POST(req: NextRequest) {
  return withRouteHandler(
    ...
    async () => {
      ...
      if (typeof body.raw !== "string" || body.raw.trim().length < 20) {
        throw new ValidationError("raw 마케팅 자료가 너무 짧아요 (20자 이상).");
      }
      const result = await geminiSop.deriveFromMarketing({ raw: body.raw });
