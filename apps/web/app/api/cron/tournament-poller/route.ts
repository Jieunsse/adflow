...
import { acquireCronLock, recordCronRun, releaseCronLock } from "@shared/lib/cron-runs";
...
export async function GET(req: NextRequest) {
  ...
  if (!(await acquireCronLock("tournament-poller"))) return NextResponse.json({ error: "이미 실행 중이에요." }, { status: 409 });
  ...
  try {
    ...
    await releaseCronLock("tournament-poller");
  }
}
