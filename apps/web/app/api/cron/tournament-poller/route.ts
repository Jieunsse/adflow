...
// running 토너먼트마다: ① Spring 에 결산을 시킨다(판정은 Java, 스터디 미확정이면 보류) ② 결산되면
// auto 무인 체인으로 다음 라운드 자동 게재 ③ owner 의 열린 SSE 스트림으로 push(연결 없으면 no-op).
//
// 단계 5 — 판정이 Java 로 넘어가면서 이 라우트는 **얇은 트리거**가 됐다(설계 §9). 남은 TS 는 게재
// (autoAdvance)와 알림뿐이고, 단계 6 에서 Spring @Scheduled 가 트리거까지 흡수하면 사라진다.
//
// 인증 = CRON_SECRET (Vercel Cron 은 authorization: Bearer ${CRON_SECRET} 로 호출). 세션 없음 —
// 게재·푸시에 필요한 토큰/계정/페이지는 토너먼트 delivery 봉투에서 읽는다.

import { NextRequest, NextResponse } from "next/server";
import {
  getRealTournamentRunner,
  tournamentStore,
  settleRoundOnBackend,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const all = await tournamentStore.list();
    ...
    for (const t of active) {
      try {
        const result = await settleRoundOnBackend(t.id);
        ...
        const fresh = await tournamentStore.get(t.id);
