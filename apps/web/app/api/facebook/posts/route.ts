...
import { FB_PAGE_POSTS_MOCK, listPagePosts } from "@/lib/facebook-posts"
...
export async function GET(req: NextRequest) {
  ...
  if (session.browseMode) return NextResponse.json({ posts: FB_PAGE_POSTS_MOCK, mock: true })
  ...
  try {
    return NextResponse.json(await listPagePosts(pageId, session.accessToken, cursor))
  } catch (error) {
    const status = error instanceof MetaGraphError && error.status >= 400 ? error.status : 502
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Facebook 게시물 조회 실패", status }, { status })
  }
}
