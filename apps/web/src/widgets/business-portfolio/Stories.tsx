...
function StoryThumb({ story }: { story: IgStory }) {
  ...
  if (!src) {
    return (
      <div className="w-full aspect-[9/16] rounded-xl bg-gradient-to-br from-[var(--w-bg-assistive)] to-[var(--w-bg-alternative)] grid place-items-center">
  ...
  return (
    <div className="relative w-full aspect-[9/16] rounded-xl overflow-hidden bg-[var(--w-bg-alternative)]">
      ...
      {story.mediaType === "VIDEO" && (
        <div
          className="absolute top-2 right-2 py-0.5 px-1.5 rounded-md bg-black/55 text-white"
          style={{ font: "600 10.5px/1 var(--w-font-sans)" }}
        >
          VIDEO
        </div>
      )}
...
export default function Stories() {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        스토리는 게시 후 24시간이 지나면 사라져요. 만료된 스토리 보기는 곧
        제공할게요.
      </div>
    </div>
  );
