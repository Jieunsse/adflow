...
| 검증 | 결과 |
|---|---|
| `./gradlew test` | **492건** green (457 → 492), Docker 없이 |
| `./gradlew integrationTest` | **24건** green (23 → 24), 실제 PostgreSQL 16 |
...
`server-runner.ts` 는 163줄로 줄었고 남은 것은 전부 저장만 하는 편집이다(챔피언 확정·재생성·수동
챌린저·봉투 충전·복구).
...
**`@Version` 이 지키지 못하는 경로가 있다.** 설계 §6 대로 낙관적 락을 걸었고 Postgres 통합 테스트가
동시 수정 검출을 확인한다(역검증도 했다 — `@Version` 을 떼면 그 테스트가 깨진다). 그런데 Next 의
**전체 애그리거트 upsert 는 지우고 새로 넣으므로 버전 비교를 지나친다.** 챔피언 확정·봉투 충전·복구
같은 편집이 아직 그 경로다 — 사람이 누르는 순간과 폴러 틱이 겹치면 앞선 변경이 조용히 사라질 수 있다.

닫는 방법은 하나다: 그 여섯 편집도 Spring 의 좁은 엔드포인트로 옮기고 Next 의 토너먼트 upsert 경로를
없앤다. 이번 단계 범위 밖이라 손대지 않았다.
