  ...
  return (
    <Card>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {browseMode ? (
          <Chip variant="violet">둘러보기 데모</Chip>
        ) : accountConnected ? (
          <Chip variant="success" dot live>연결됨</Chip>
        ) : (
          <Chip variant="warn" dot>미연결</Chip>
        )}
      </div>
...
function SumRow({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="flex justify-between py-2.5 border-b border-[var(--w-line-alternative)]">
      <span className="font-medium text-[13px] leading-none text-[var(--w-fg-normal)]">{label}</span>
      <div className="text-right">
        <span className="font-semibold text-[13px] leading-none text-[var(--w-fg-strong)]">{value}</span>
        {sub && <div className="font-medium text-[12px] leading-none font-[var(--w-font-mono)] text-[var(--w-fg-neutral)] mt-1">{sub}</div>}
      </div>
    </div>
  );
}
